...
## [2026-05-26] Alteração de UI - Remoção do Nome Duplicado no Menu Suspenso de Perfil
- **Resumo**: Removemos o bloco que exibia o nome do usuário no topo do menu suspenso (Dropdown) que abre ao clicar no avatar de perfil. Essa alteração foi aplicada tanto no menu do cabeçalho de desktop (`src/components/layout/NavigationRail.tsx`) quanto no cabeçalho de dispositivos móveis (`src/components/layout/MobileTopHeader.tsx`).
- **Motivação**: Atender à solicitação direta do usuário para remover a duplicação visual do nome/apelido dentro do menu de perfil, visto que a saudação com o nome do usuário ("Boa tarde, Mendonça's") já é exibida de forma permanente e proeminente no cabeçalho principal logo ao lado do botão de avatar nas duas interfaces.
