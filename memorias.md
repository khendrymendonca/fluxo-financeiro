...
- **Motivação**: Atender ao cerne estratégico do Fluxo como um gestor inteligente e simplificado de finanças pessoais, fornecendo um diagnóstico profissional, prognósticos de quitação detalhados, justificativas de comportamento do Fluxo Score e visões claras de estouros de orçamentos e parcelamentos em um PDF gerencial corporativo.
  ...
  45. **Correção: Crash do Radix UI causado pelo Google Translate + 401 no auto-fix de categorias (26/08/2026)**:
      - **Bug `removeChild` / Google Translate**: Adicionado `translate="no"` e `lang="pt-BR"` na tag `<html>` do [index.html](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/index.html). O Google Translate interceptava o DOM envolvendo nós de texto em `<font>` tags; quando o Radix UI tentava desmontá-los, o `removeChild` falhava com `NotFoundError`, derrubando a árvore React e exibindo "Algo deu errado". O `translate="no"` instrui o browser a não modificar o DOM da página.
      - **Bug 401 no auto-fix de `categories`**: O bloco fire-and-forget dentro do `queryFn` de `useTransactions` em [useFinanceQueries.ts](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/src/hooks/useFinanceQueries.ts) disparava queries ao Supabase sem verificar se a sessão estava ativa. Quando o token JWT expirava, as queries retornavam 401. Corrigido com `supabase.auth.getSession()` antes das queries e tratamento explícito de erros de autenticação (código `PGRST301` / status `401`) para abortar silenciosamente.

  46. **Substituição da Tela Exportar por Análise de Custos (31/08/2026)**:
      - **Exclusão da Tela Exportar**: O componente legado `ExportManager.tsx` e seu acesso foram removidos da interface do app.
      - **Criação da Tela Análise de Custos (`CostAnalysisPage.tsx`)**: Implementada uma tela 100% isolada e modular dedicada à análise detalhada de custos por categoria e subcategoria, sem afetar nenhuma regra das outras telas do sistema.
      - **Visão Real de Custo**: Captura os gastos reais do item selecionado independente do meio de pagamento (dinheiro, débito, boleto ou cartão de crédito parcelado/à vista), permitindo mensuração exata de custos recorrentes ou pontuais (ex: energia elétrica, gastos com filhos, alimentação).
      - **Lógica de Subcategoria Dinâmica**: Quando o usuário seleciona uma subcategoria específica, toda a tela calcula exclusivamente as métricas dela. Caso selecione "Todas as Subcategorias", a tela calcula o montante consolidado da categoria principal e expõe um bloco executivo com gráfico Donut e tabela percentual de participação de cada subcategoria.
      - **Ajuste Estatístico de Médias (31/08/2026)**: As médias (diária e mensal, especialmente no filtro Anual ou Semestral) agora adotam rigor estatístico identificando o primeiro mês com gasto registrado na categoria/subcategoria selecionada como ponto de partida da amostragem, evitando que meses vazios anteriores reduzam a média artificialmente.
      - **Remoção da Opção 'Todas as Categorias'**: O dropdown de categorias agora lista estritamente as categorias reais cadastradas, eliminando a opção inválida 'Todas as categorias' e mantendo foco no item ativo.
  47. **Edição em Massa de Categoria e Subcategoria no Extrato com Propagação para Gestão de Contas (31/08/2026)**:
      - **Ação em Lote no Extrato**: Na tela de lançamentos/extrato ([TransactionList.tsx](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/src/components/transactions/TransactionList.tsx)), ao ativar a seleção e marcar múltiplos lançamentos, a barra de ação flutuante agora exibe o botão **Alterar Categoria** (com ícone `Tags`).
      - **Modal Elegante de Alteração em Lote (`BulkCategoryDialog.tsx`)**: Modal intuitivo e sem bloqueios que utiliza o seletor visual `CategoryPickerPopover`, permitindo escolher a categoria de destino e opcionalmente a subcategoria correspondente (ou mantê-la sem subcategoria).
      - **Propagação e Alinhamento na Gestão de Contas e Parcelamentos (`useBulkUpdateTransactionCategory`)**: Em [useTransactionMutations.ts](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/src/hooks/useTransactionMutations.ts), a mutação detecta se algum dos lançamentos selecionados pertence a um parcelamento (`installmentGroupId`), a uma dívida/acordo (`debtId`) ou a uma conta fixa/recorrente (`originalId` ou `isRecurring`).
      - **Atualização Global Coerente**: Atualiza todas as parcelas reais vinculadas no Supabase e a conta mãe/filhas recorrentes. Graças ao motor reativo de [useProjectedTransactions.ts](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/src/hooks/useProjectedTransactions.ts), as parcelas e meses futuros projetados (virtuais) herdam instantaneamente a nova categoria e subcategoria, mantendo todo o fluxo financeiro 100% alinhado e sem inconsistências contábeis.

  48. **Proteção e Travamento das Categorias de Sistema UI e Banco (01/09/2026)**:
      - **Resolução de BUG (Update Mutação)**: Corrigido problema onde não era possível retirar a subcategoria de um lançamento individual pois a string vazia era filtrada pela instrução de delete-undefined no update do Supabase em [useTransactionMutations.ts](file:///C:/Users/khendry.mendonca/OneDrive%20-%20TORP%20INDUSTRIA%20TEXTIL%20LTDA/Projeto/fluxo-financeiro/src/hooks/useTransactionMutations.ts). Agora permite repassar `null`.
      - **Bloqueio Completo de Categorias Fixas (isSystem)**: As categorias "Cartão de Crédito", "Abatimento no Cartão", "Acordo" e "Transferência" agora possuem bloqueios duplos. No banco, foi gerada e aplicada a migração [0056_retroactive_categories.sql](file:///C:/Users/khendry.mendonca/OneDrive%20-%20TORP%20INDUSTRIA%20TEXTIL%20LTDA/Projeto/fluxo-financeiro/supabase/migrations/0056_retroactive_categories.sql) forçando a bandeira `is_system = true` nestas categorias ativas de forma retroativa e repassando o ID explícito para todos os pagamentos e abates históricos.
      - **Ocultação Front-end**: As categorias marcadas como sistema estão sendo ocultadas dos seletores para o usuário não as criar indevidamente (`CategoryPickerPopover.tsx` e `useTransactionFormState.ts`). Para reforço, o componente [CategoriesManager.tsx](file:///C:/Users/khendry.mendonca/OneDrive%20-%20TORP%20INDUSTRIA%20TEXTIL%20LTDA/Projeto/fluxo-financeiro/src/components/settings/CategoriesManager.tsx) possui agora validação na digitação que impede cadastrar ou renomear tanto categorias quanto subcategorias com esses nomes reservados de sistema, emitindo um toast de aviso.
      - **Otimização de UI (Responsividade)**: O botão "Remover lançamentos" que ativava o modo de seleção em massa foi removido do topo da tela de extrato para liberar espaço e resolver quebras de layout em telas menores. A exclusão agora depende 100% da seleção contextual: o usuário entra no modo de seleção em massa simplesmente tocando/hovering nas transações (checkboxes) e acionando o menu flutuante (action bar) na parte inferior da tela, que mantém todas as regras de integridade e travamento do sistema.
...
- 2026-09-03: Implementada arquitetura rigorosa para a categoria 'Ajuste de Saldo' (migration 0056). A categoria agora é criada nativamente via trigger no banco (is_system = true, tipo genérico 'expense' mas exibida para ambas as abas no frontend) garantindo consistência estrita no banco entre ajustes manuais e automáticos, e ocultando a categoria da edição na tela de gestão.

## HISTÓRICO DE ATUALIZAÇÕES
- 2026-09-04: Ajustes de UI/UX implementados: Remoção do fundo preto no hover dos gráficos de barras (CostAnalysisPage e ReportsDashboard) para visibilidade dos valores. Transformação dos filtros de bancos e contas na lista de lançamentos de botões lado-a-lado para menu suspenso (Select), economizando espaço mobile. Substituição de overflow horizontal por flex-wrap responsivo nos filtros de lançamentos e gestão de contas.
  - Ajuste de botões de ações (editar, baixar, excluir) na Gestão de Contas para ocupar menos espaço em telas muito estreitas (344px), tornando-os quadrados apenas com o ícone no mobile.
  - Correção adicional de responsividade na tela de Gestão de Contas (celulares extremamentes estreitos): remoção de overflow horizontal causado por larguras mínimas em botões de filtros (min-w-100px), redução dos botões de ação para 36x36px (h-9 w-9) no mobile, e adição de flex-wrap e ajustes de padding (p-3) nos cards das contas para garantir visibilidade total sem rolagem lateral.
  - Adicionado min-w-0 e flex-wrap nos cabeçalhos e filtros (PageHeader, MonthSelector e input de busca) que forçavam a tela a expandir além de 344px em telas muito estreitas.
  - 2026-09-04: Bloqueio estrito de overflow horizontal global (html, body, #root, AppLayout) e reestruturação do card de lançamentos na Gestão de Contas em duas linhas perfeitas no mobile (Linha 1: Ícone, Título e Tags / Linha 2: Valor à esquerda e Ações 32x32px à direita), garantindo visualização completa em telas a partir de 300px (ex: Galaxy Z Fold 5).

- 2026-09-23: Refatoração na tela de Análise de Custos para permitir múltipla seleção de subcategorias. Os menus de seleção suspensos de categoria e subcategoria foram substituídos por grades responsivas (botões toggle), garantindo acesso mais rápido com um clique. A área de detalhamento em gráfico de pizza (Donut) e a lista de participação percentual não somem mais ao filtrar. O gráfico de barras foi aprimorado para remover o fundo branco indesejado ao passar o mouse e incluir os rótulos de valores visíveis diretamente nas barras (LabelList), otimizando a leitura das informações.
  - Ajuste de Formatação (23/09/2026): O gráfico de barras e seus eixos (YAxis e LabelList) agora renderizam os valores estritamente no padrão monetário brasileiro (R$ e vírgula na casa decimal), resolvendo a falta de formatação apropriada nas etiquetas.
  - Implementação de Teto de Gastos (Orçamento) para Subcategorias (23/09/2026): O sistema agora permite definir orçamentos específicos também para subcategorias na tela de Gestão de Categorias, com botões rápidos para cálculo percentual baseado no teto da categoria-mãe. Na tela de Análise de Custos, o limite orçamentário ativo (categoria ou subcategoria) agora exibe uma barra de progresso executiva no card de Total Gasto, permitindo ao usuário comparar imediatamente o consumo contra o seu planejamento financeiro. Adicionada migração de banco (0057_add_budget_limit_to_subcategories.sql).
  - Ajuste Visual Orçamento (23/09/2026): Adicionada uma linha de referência (ReferenceLine) tracejada vermelha com o rótulo 'Orçamento' nos gráficos da tela de Análise de Custos, indicando visualmente o teto de gastos definido para a categoria/subcategoria selecionada.
  - Ajuste de Responsividade do Gráfico (23/09/2026): Removida a margem negativa que cortava os valores do eixo Y (vertical) em telas pequenas. Os valores do eixo foram limpos para exibir o número cru e abreviado (sem o prefixo 'R$') a fim de economizar espaço valioso em dispositivos mobile. Além disso, a margem superior foi estendida para garantir que a linha de teto ('Orçamento') e os valores acima das barras não fossem sobrepostos e ocultos.

  - Limpeza de Interface (24/09/2026): Remoção completa da tela de Exportar Dados do app mobile e desktop. O sistema de Notificações Push foi inteiramente desativado e removido (incluindo abas do SuperPage, configurações e listeners do Service Worker). Adicionalmente, a tela de Análise de Custos foi reinserida na navegação mobile, e o layout do Menu Sanduíche com avatar alinhado e botão central flutuante foi consolidado.
