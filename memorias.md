...
12/04/2026 | Bugfix Projeções | Refino crítico no motor de virtualização: Escopo 'this' cria filha física ao invés de editar a mãe; escopos 'future' e 'all' têm `date` protegida contra sobrescrita na mãe; BillsManager libera renderização de edição para transações virtuais; `EditBillForm` passa id `-virtual-` completo para o Supabase mutation. Build avança para série AB-10.
