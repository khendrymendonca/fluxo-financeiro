...
- **MotivaÃ§Ã£o**: Atender ao cerne estratÃ©gico do Fluxo como um gestor inteligente e simplificado de finanÃ§as pessoais, fornecendo um diagnÃ³stico profissional, prognÃ³sticos de quitaÃ§Ã£o detalhados, justificativas de comportamento do Fluxo Score e visÃµes claras de estouros de orÃ§amentos e parcelamentos em um PDF gerencial corporativo.
  ...
  37. **Correção de UI/Regra de Negócio: Exibição de Faturas Atrasadas na Gestão de Contas (05/08/2026)**:
      - **Ajuste na Geração de Faturas Virtuais**: Corrigimos o utilitário `buildCardInvoiceObligations` em [invoiceObligations.ts](file:///C:/Users/khendry.mendonca/OneDrive - TORP INDUSTRIA TEXTIL LTDA/Projeto/fluxo-financeiro/src/utils/invoiceObligations.ts). Anteriormente, a função gerava faturas virtuais apenas para o mês selecionado (`viewDate`). Isso fazia com que faturas de meses anteriores que estivessem atrasadas e não pagas deixassem de ser calculadas e exibidas quando o usuário mudava de mês (por exemplo, ao visualizar Agosto, a fatura de Julho atrasada desaparecia).
      - **Geração Abrangente de Pendências**: Agora, a função localiza todas as competências únicas (`invoiceMonthYear`) em que o cartão de crédito possui lançamentos, e calcula faturas virtuais para todos os meses menores ou iguais ao mês selecionado. Se a fatura possuir saldo restante (`remainingAmount > 0`), a obrigação virtual é gerada de forma independente usando um ID específico do mês correspondente (`fat-virtual-${card.id}-${monthStr}`), fazendo com que apareça como "Atrasado" nos meses subsequentes até ser liquidada.
      - **Motivação**: Atender ao bug relatado onde faturas não-pagas de meses anteriores sumiam da Gestão de Contas ao navegar para o mês seguinte, garantindo que todas as pendências financeiras reais permaneçam visíveis e em destaque até sua quitação.
