...
08/04/2026	Projeto Global	Restrição do tema AMOLED exclusivamente para a versão Mobile. Em dispositivos Web/Desktop (viewport >= 1024px), a opção foi removida da UI e o hook `useTheme` agora realiza downgrade automático para 'dark' caso o AMOLED esteja selecionado, mantendo a consistência visual. Build da aplicação avançou para a versão 'P'.
