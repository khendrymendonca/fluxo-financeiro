...
- **Motivação**: Eliminar poluição visual nas telas de Lançamentos e Relatórios por meio de componentes estruturados e controles deslizantes no padrão Apple, introduzir a filtragem secundária por subcategorias, facilitar a identificação de contas correntes associadas aos seus respectivos bancos e personalizar os cartões de compromissos com base no status de vencimento.
